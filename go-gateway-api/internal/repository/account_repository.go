...
// NewAccountRepository cria um novo AccountRepository
...
// Save salva uma nova conta no banco de dados
...
func (r *AccountRepository) UpdateBalance(account *domain.Account) error {
	...
	_, err = tx.Exec(`UPDATE accounts SET balance = $1, updated_at = $2 WHERE id = $2`, account.Balance,
		time.Now(), account.ID)
	if err != nil {
		return err
	}
